...
// Daca nu sunt transmise date prin $_GET['pg']
// Incheie executia scriptului
if (!isset($_GET['pg'])) {
	exit;
}

// Preia din URL pagina curenta, in format "urlencode"
$pagina = urlencode(strip_tags($_GET['pg']));
...
$nume_cookie = preg_replace('/[^A-Za-z0-9_]/', '', strip_tags($_GET['pg']));
...
// Afiseaza functia JS cu datele de iesire
echo 'document.write("Vizitatori - <b><i>'. $vizitatori. '</i></b><br />Accesari &nbsp; - <b><i>'. $accesari. '</i></b>")';
?>
